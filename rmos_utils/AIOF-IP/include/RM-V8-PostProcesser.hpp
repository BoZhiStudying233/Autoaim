#ifndef RM_V8_POST_PEOCESSER_HPP_
#define RM_V8_POST_PEOCESSER_HPP_

#include "Module.hpp"

class RmV8PostProcesser : public PostProcesser
{
...
public:
    ...
    void process(DetectResultList & output) override 
    {
        /*
            [1 * 5460 * 20]
            x y w h [class conf 6] x1 y1 x2 y2 x3 y3 x4 y4
        */     
        ...
        for (size_t i = 0; i < tensor_shape_[1]*tensor_shape_[2]; i+=tensor_shape_[2])
        {
            ...
            double max_conf = 0.0;
            int max_id = 0;
            for (int j = 4 ; j < tensor_shape_[2]- key_points_num_ * 2; j++)
            {
                if (p_tensor_[i + j] >= max_conf)
                {
                    max_conf = p_tensor_[i + j];
                    max_id = j - 4;
                }
            }
            // std::cout << "max_conf: " << max_conf << " max_id: " << max_id << std::endl;
            ...
            DetectResult pre_result = {
                ...
                .points = {
                        cv::Point2d(
                                (p_tensor_[i + 10] - remap_params_.ox) / remap_params_.scale, 
                                (p_tensor_[i + 11] - remap_params_.oy) / remap_params_.scale
                            ),
                        cv::Point2d(
                                (p_tensor_[i + 12] - remap_params_.ox) / remap_params_.scale, 
                                (p_tensor_[i + 13] - remap_params_.oy) / remap_params_.scale
                            ),
                        cv::Point2d(
                        ...
                        cv::Point2d(
                                (p_tensor_[i + 16] - remap_params_.ox) / remap_params_.scale, 
                                (p_tensor_[i + 17] - remap_params_.oy) / remap_params_.scale
                            ),
                        cv::Point2d(
                                (p_tensor_[i + 18] - remap_params_.ox) / remap_params_.scale, 
                                (p_tensor_[i + 19] - remap_params_.oy) / remap_params_.scale
                            )
                    },
                .id = max_id,
                .conf = max_conf
