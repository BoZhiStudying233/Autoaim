...
namespace tool
{
    ...
    void Tracker::initParams(TrackerParams tracker_params)
    {
        this->tracker_params_ = tracker_params;
    }
    ...
    void Tracker::update(std::vector<base::Armor> armors)
    {
        ...
        if (!armors.empty()) {
            ...
            if (min_position_diff < this->tracker_params_.max_match_distance_ && yaw_diff < this->tracker_params_.max_match_yaw_diff_) {
                // Matched armor found
                matched = true;
            ...
            } else if (same_id_armors_count == 1 && yaw_diff > this->tracker_params_.max_match_yaw_diff_) {
        ...
        if (tracker_state == base::DETECTING) {
            if (matched) {
                detect_count_++;
                if (detect_count_ > this->tracker_params_.tracking_thres) {
                    detect_count_ = 0;
            ...
            if (!matched) {
                lost_count_++;
                if (lost_count_ > this->tracker_params_.lost_thres) {
                    lost_count_ = 0;
                    tracker_state = base::LOST;
                }
            } else {
    ...
    void Tracker::handleArmorJump(const base::Armor & a)
    {
        ...
        if ((current_p - infer_p).norm() > this->tracker_params_.max_match_distance_) {
