...
namespace rmos_detector
{
    ...
    void MixDetectorNode::setMode(int mode)
    {
        if(mode == 1)
            this->mode_ = base::Mode::NORMAL_RUNE;//小符模式
        else if(mode == 2)
            this->mode_ = base::Mode::RUNE;
        else if(mode == 0)
            this->mode_ = base::Mode::NORMAL;//normal指的是自瞄模式
        
        else
            this->mode_ = this->last_mode_;// wait状态
...
RCLCPP_COMPONENTS_REGISTER_NODE(rmos_detector::MixDetectorNode)
